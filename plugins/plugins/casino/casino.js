...
let casinoOwners = JSON.parse(fs.readFileSync(casinoAuthDataFile).toString());
let defaultPermission = 'ban';
...
function checkBingo(room) {
	...
	for (let i in tables) {
		...
		tableComplete = 0;
		for (let j = 0; j < targetTable.length; ++j) {
	...
	if (endGame) {
		let winData = '';
		for (let n = 0; n < (winners.length - 1); ++n) {
...
exports.commands = {
	...
	buytable: function (target, room, user) {
		...
		let numbers = getBingoNumbers().randomize();
		let cells = [];
		for (let i = 0; i < 5; ++i) {
	...
	bingo: function (target, room, user) {
		...
		if (tables[targetUserId]) {
			...
			for (let n = 0; n < tables[targetUserId].length; ++n) {
	...
	tourbet: function (target, room, user) {
		...
		let tourUsers = Tournaments.tournaments[room.id].generator.getUsers();
		let isInTour = false;
	...
	newwheel: function (target, room, user) {
		...
		let initPrize = parseInt(params[0]);
		let wheelSize = parseInt(params[1]);
		...
		for (var i = 0; i < keys.length && pokemonLeft < wheelSize; i++) {
			let template = Tools.getTemplate(keys[i]);
	...
	endwheel: function (target, room, user) {
		...
		let pkm = wheelOptions[Math.floor(Math.random() * wheelOptions.length)];
		let htmlDeclare = '<div class="broadcast-green"><center><h1>Juego de Ruleta Finalizado</h1><h3>La ruleta ha girado y el Pokemon elegido es ' + pkm + '</h3><img src="http://play.pokemonshowdown.com/sprites/xyani/' + toId(pkm) + '.gif" title="' + pkm + '" /> <br /><br /><b>';
		let winners = [];
		for (let i in wheelBets) {
	...
	addcasinomoney: function (target, room, user) {
		...
		let money = Shop.getUserMoney(user.name);
		let targetMoney = parseInt(target);
	...
	tragaperras: function (target, room, user) {
		...
		Shop.removeMoney(user.name, money);
		let slotSymbols = ["\u2605", "\u2665", "@", "%", "$", "&", "#", "+", "~"];
		let symbolA = slotSymbols[Math.floor(Math.random() * slotSymbols.length)];
		let symbolB = slotSymbols[Math.floor(Math.random() * slotSymbols.length)];
		let symbolC = slotSymbols[Math.floor(Math.random() * slotSymbols.length)];
	...
	casino: function (target, room, user) {
		...
		if (!owners || owners.length < 1) {
			...
			for (let n = 1; n < (owners.length - 1); ++n) {
