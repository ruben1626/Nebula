...
const ARGUMENTS_REGEXP = new Map([
	[3, /^\s*(([0-9]+\.)?[0-9]+([°%])?)\s*,\s*(([0-9]+\.)?[0-9]+([°%])?)\s*,\s*(([0-9]+\.)?[0-9]+([°%])?)\s*$/],
	[4, /^\s*(([0-9]+\.)?[0-9]+([°%])?)\s*,\s*(([0-9]+\.)?[0-9]+([°%])?)\s*,\s*(([0-9]+\.)?[0-9]+([°%])?)\s*,\s*(([0-9]+\.)?[0-9]+([°%])?)\s*$/],
]);
...
function parseHEX(input) {
	...
	switch (input.length) {
	case 3: return Color.fromHEX(input.slice(0, 1), input.slice(1, 2), input.slice(2, 3), 1);
	case 4: return Color.fromHEX(input.slice(0, 1), input.slice(1, 2), input.slice(2, 3), input.slice(3, 4));
...
function parseHSL(input) {
	const parts = ARGUMENTS_REGEXP.match(input);
	if (!parts) return null;
	const hue = Number(parts[0].endsWith('°') ? parts[0].slice(0, -1) : parts[0]);
	const sat = Number(parts[1].endsWith('%') ? parts[1].slice(0, -1) : parts[1]);
	const lum = Number(parts[2].endsWith('%') ? parts[2].slice(0, -1) : parts[2]);
...
exports = module.exports = function (input) {
	...
	if (FUNCTION_REGEXP.test(input)) {
		if (input.startsWith('hsl(')) return parseHSL(input.slice(4));
		if (input.startsWith('hsla(')) return parseHSLA(input.slice(5));
		if (input.startsWith('rgb(')) return parseRGB(input.slice(4));
		if (input.startsWith('rgba(')) return parseRGBA(input.slice(5));
		return null;
	}
	return null;
};
