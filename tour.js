	...
	let tourStuff = {
		...
		reportdue: function (room, connection) {
			...
			if (trid.players.length == trid.playerslogged.length) {
				...
				var someid = trid.players[trid.playerslogged.length];
				let prelistnames = '<b>' + tour.username(someid) + '</b>';
				...
				let listnames = prelistnames + ' y <b>' + tour.username(someid) + '</b>';
		...
		start: function (rid) {
			...
			do {
				let numPlayers = ((tour[rid].size - numByes) / 2 + numByes);
				do {
			...
			while (numByes > 0);
			do {
				let match = new Array(); //[p1, p2, result]
		...
		startRaw: function (i) {
			...
			for (var i in round) {
				if (!round[i][1]) {
					...
					var p1n = tour.username(round[i][0]);
					let p2n = tour.username(round[i][1]);
					if (p1n.substr(0, 6) === 'Guest ') p1n = round[i][0];
		...
		nextRound: function (rid) {
			...
			if (w.length == 1) {
				...
				if (tour[rid].size >= 3 && Rooms.rooms[rid].isOfficial) {
					let moneyFirst = tour[rid].size * 10;
					let moneySecond = Math.floor(moneyFirst / 2);
				...
				let html = '<hr /><h3><font color="green">Ronda ' + tour[rid].roundNum + '!</font></h3><font color="blue"><b>FORMATO:</b></font> ' + Tools.data.Formats[tour[rid].tier].name + "<hr /><center> <small><font color=red>Rojo</font> = descalificado, <font color=\"green\">Green</font> = paso a la siguiente ronda, <a class='ilink'><b>URL</b></a> = combatiendo</small>";
				let pBye = new Array();
				let pNorm = new Array();
				let p = new Array();
				for (var i in w) {
				...
				for (var i = 0; i < p.length / 2; i++) {
					...
					let p1n = tour.username(p[p1]);
					let p2n = tour.username(p[p2]);
...
let cmds = {
	...
	dq: function (target, room, user, connection) {
		...
		let targetUser = Users.get(target);
		if (!targetUser) {
			var dqGuy = Chat.escapeHTML(target.toLowerCase());
		} else {
	...
	replace: function (target, room, user, connection) {
		...
		let outof = ["players", "winners", "losers", "round"];
		for (let x in outof) {
			for (let y in rt[outof[x]]) {
				let c = rt[outof[x]][y];
				if (outof[x] == "round") {
					if (c[0] == t[0]) c[0] = t[1];
					if (c[1] == t[0]) c[1] = t[1];
	...
	survey: function (target, room, user) {
		...
		let pollOptions = '';
		for (let i = 0; i < tour[room.id].answerList.length; ++i) {
	...
	endsurvey: function (target, room, user) {
		...
		let votes = Object.keys(tour[room.id].answers).length;
