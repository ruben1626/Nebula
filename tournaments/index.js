...
class Tournament {
	...
	onTournamentEnd() {
		...
		if (global.Shop) {
			let data = {results: this.generator.getResults().map(usersToNames), bracketData: this.getBracketData()}; data = data['results'].toString(); let runnerUp = false; let winner = false; if (data.indexOf(',') >= 0) {data = data.split(','); winner = data[0]; if (this.room.id === 'casino' && Users.get(Bot.config.name)) Chat.parse('/endtourbets ' + winner, this.room, Users.get(Bot.config.name), Users.get(Bot.config.name).connections[0]); if (data[1]) runnerUp = data[1];} else {winner = data;} let tourSize = this.generator.users.size; if (this.room.isOfficial && tourSize >= 3) {let firstMoney = tourSize * 20; let secondMoney = Math.floor((tourSize * 20) / 2); let firstBuck = 'PokeDolar'; let secondBuck = 'PokeDolar'; if (firstMoney > 1) firstBuck = 'PokeDolares'; if (secondMoney > 1) secondBuck = 'PokeDolares'; Shop.giveMoney(winner, firstMoney); this.room.add('|raw|<strong>' + Tools.escapeHTML(winner) + '</strong> ha ganado ' + firstMoney + ' ' + firstBuck + ' por ganar el torneo!'); if (runnerUp) { Shop.giveMoney(runnerUp, secondMoney); this.room.add('|raw|<strong>' + Tools.escapeHTML(runnerUp) + '</strong> tambi�n ha ganado ' + secondMoney + ' ' + secondBuck + ' por quedar en segundo lugar!');}}
		}
		this.isEnded = true;
		if (this.autoDisqualifyTimer) clearTimeout(this.autoDisqualifyTimer);
